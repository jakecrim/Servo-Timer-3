...
{ 
  ...
  {
	TCNT3 =  servos[Channel].remainder;   // yes, set count for overflow after remainder ticks
  }  
  else if(ISRCount >  servos[Channel].counter)  
  {
	...
	TCNT3 = 0;    // reset the clock counter register
...
{   
	...
	/* setup for timer 2 */
	TIMSK3 = 0;  // disable interrupts 
	TCCR3A = 0;  // normal counting mode 
	TCCR3B = _BV(CS31); // set prescaler of 8 
	TCNT3 = 0;     // clear the timer2 count 
	TIFR3 = _BV(TOV3);  // clear pending interrupts; 
	TIMSK3 =  _BV(TOIE3) ; // enable the overflow interrupt	  
